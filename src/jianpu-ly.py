#!/usr/bin/env python3
# c.f. https://ssb22.user.srcf.net/mwrhome/jianpu-ly.html
...
def score_end(**headers):
    ...
    if headers:
        ...
        ret += "}\n"
    if midi:
        # TODO: make this customisable (and/or check how to print BPMs in jianpu)
...
assert not (
    use_rest_hack and not dashes_as_ties), "This combination has not been tested"
...
class notehead_markup:
    ...
    def endScore(self):
        if not self.barPos == self.startBarPos:
            errExit("Incomplete bar at end of score %d (pos %d, should be %d)" % (
                scoreNo, self.barPos, self.startBarPos))
    ...
    def setAnac(self, denom, dotted):
        ...
        if not self.barPos:
            errExit("Anacrusis should be shorter than bar in score %d" % scoreNo)
    ...
    def __call__(self, figures, nBeams, dot, octave, accidental):
        ...
        # octave is "", "'", "''", "," or ",,"
        # accidental is "", "#", "b"
        if len(figures) > 1 and accidental:
            # see TODOs below
            errExit("Accidentals in chords not yet implemented")
        ...
        def get_placeholder_chord(figures):
            if len(figures) == 1:
                ...
                # (assuming all in middle octave for now, TODO how to correctly interpret octaves w.chords)
        ...
        (grob-interpret-markup grob
          """ % self.defines_done[figures]
            if len(figuresNew) == 1 or figures.startswith("-"):
                ret += """(make-lower-markup 0.5 (make-bold-markup "%s")))))))
""" % figuresNew
            else:
...
"""  # TODO: can do accidentals e.g. #:halign 1 #:line ((#:fontsize -5 (#:raise 0.7 (#:flat))) (#:bold "3")) but might cause the beam not to extend its full length if this chord occurs at the end of a beamed group, + accidentals won't be tracked by Lilypond and would have be taken care of by jianpu-ly (which might mean if any chord has an accidental on one of its notes we'd have to do all notes in that bar like this, whether they are chords or not)
        ...
        if figures == "-" or all('1' <= figure <= '7' and not accidental == self.current_accidentals[octave][int(figure)-1] for figure in list(figures)) and nBeams > self.lastNBeams:
        ...
        # TODO: if figures=="0" then that might be typeset a bit to the left as well (because it's also a rest), however extending the line TOO far left in this case could be counterproductive
            ...
            leftBeams = 0
        if leftBeams:
            assert nBeams, "following logic assumes if (leftBeams or nBeams) == if nBeams"
        aftrlast0 = ""
        ...
        # TODO: this will fail if use accidental with chord (see accidentals-in-chords TODOs above)
        ret += {"": "", "#": "is", "b": "es"}[accidental]
        if len(placeholder_chord) == 1 and not placeholder_chord == "r":
            ret += {"": "'", "'": "''", "''": "'''", ",": "", ",,": ","}[
                octave]  # for MIDI + Western, put it so no-mark starts near middle C (TODO: how to interpret octaves with chords?  octave applies to ALL notes in the chord?  for now we just assume all chord-notes are middle octave for the MIDI)
        length = 4
        b = 0
        toAdd = F(16)  # crotchet
        while b < nBeams:
        ...
        if self.barPos > self.barLength:
            errExit("(notesHad=%s) barcheck fail: note crosses barline at \"%s\" with %d beams (%d skipped from %d to %d, bypassing %d), scoreNo=%d barNo=%d (but the error could be earlier)" % (
                ' '.join(self.notesHad), figures, nBeams, toAdd, self.barPos-toAdd, self.barPos, self.barLength, scoreNo, self.barNo))
    ...
    for line in __doc__.split("\n"):
        ...
        if ":" in line and line.split(":", 1)[1].strip():
            ...
            if re.match(r".*[A-Za-z]\)$", shouldType):
                ...
                note = ""
            if "--html" in sys.argv:
                print("<tr><td>"+toGet.strip()+"</td><td><kbd>" +
                      shouldType.strip()+"</kbd>"+note+"</td>")
            else:
...
for f in sys.argv[1:]:
    try:
        try:
            ...
            inDat.append(open(f, encoding="utf-8").read())
        except:
...
if type("") == type(u""):  # Python 3: please use UTF-8 for Lilypond, even if the system locale says something else
    ...
    # for print() (and keep a reference to the old one in case of overzealous gc)
...
for i in xrange(len(inDat)):
    ...
    if inDat[i].startswith(r'\version'):
        errExit("jianpu-ly does not READ Lilypond code.\nPlease see the instructions.")
    ...
    for i in xrange(len(notes)):
        ...
        if n == '#':
            ...
            if notes[i:i+2] == ",,":
                ...
                below = "."
            r.append(
                r"\override #'(baseline-skip . 1.0) \center-column { \line { ")
            aftrNext = r"} \line { \pad-to-box #'(0 . 0) #'(-0.2 . 0) " + \
                '"'+below+'" } }'
        else:
...
def getLY(score):
    ...
    for line in score.split("\n"):
        ...
        if line.startswith("LP:"):
            ...
            escaping = 1
            if len(line) > 3:
                out.append(line[3:])  # remainder of current line
            ...
            for word in line.split():
                if '=' in word:  # e.g. 1=C; mark
                    ...
                    notehead_markup.barPos = (
                        notehead_markup.barPos + (newBarPos-oldBarPos)*multiplier) % notehead_markup.barLength
       ...
       (#:path
        ...
        (list (list (quote moveto) 0 0)
              ...
              (list (quote curveto) (* textWidth 0.5) -1 (* textWidth 0.5) -1 textWidth -1)))))))))))) """)
                    ...
                    if midi or western:
                        out[lastPtr] = r" \afterGrace { " + out[lastPtr] + \
  ...
  (#:halign -4
   (#:override
       ...
       (list (list (quote moveto) 0 0)
             ...
             (list (quote curveto) (* textWidth 0.5) -1 (* textWidth 0.5) -1 0 -1)))))))))))) """ + out[lastPtr]
                    ...
                    if figures:
                        ...
                        b4last, aftrlast, this, need_space_for_accidental = notehead_markup(
                            figures, nBeams, dot, octave, accidental)
                        if b4last:
                        ...
                        errExit("Unrecognised command " +
                                word+" in score "+str(scoreNo))
    if notehead_markup.barPos == 0 and notehead_markup.barNo == 1:
        errExit("No jianpu in score %d" % scoreNo)
    if notehead_markup.inBeamGroup and not midi and not western and not notehead_markup.inBeamGroup == "restHack":
    ...
    if western:  # collapse tied notes into longer notes
        out = re.sub(
            r"(?P<note>[^ ]*)4 ~ (?P=note)4 ~ (?P=note)4 ~ (?P=note)4", r"\g<1>1", out)
        out = re.sub(
            r"(?P<note>[^ ]*)4 ~ (?P=note)4 ~ (?P=note)4", r"\g<1>2.", out)
...
for score in re.split(r"\sNextScore\s", " "+inDat+" "):
    ...
    # the occasional false positive doesn't matter: has_lyrics==False is only an optimisation
    ...
    for midi in [0, 1]:
        ...
        if midi:
            ...
            if notehead_markup.withStaff:
                western = True
                print(western_staff_start()+" "+getLY(score)
                      [0]+" "+western_staff_end())
                western = False
                lyrics = lyrics.replace(
                    r'\lyricsto "jianpu"', r'\lyricsto "5line"')
            if lyrics:
                print(lyrics)
